    ...
    public class DB
    {
        ...
        public async Task AddCategory(CategoryData categoryData)
        {
            ...
            if (categoryData.parentCategory == null) {
                DB.partitionKey = categoryData.id;
                categoryData.PartitionKey = DB.partitionKey;
            }    
            ...
            try
            {
                ...
                ItemResponse<Category> aResponse =
                    await this.container.ReadItemAsync<Category>(
                        category.Id,
                        new PartitionKey(DB.partitionKey)
                    );
            ...
            catch (CosmosException ex) when (ex.StatusCode == HttpStatusCode.NotFound)
            {
                ...
                ItemResponse<Category> aResponse =
                    await this.container.CreateItemAsync<Category>(
                        category,
                        new PartitionKey(DB.partitionKey)
                    );
                ...
                if (categoryData.categories != null)
                {
                    foreach (var subCategoryData in categoryData.categories)
                    {
                        ...
                        subCategoryData.PartitionKey = DB.partitionKey;
        ...
        //            new CosmosClientOptions()
        //            {
        //                ApplicationName = "CosmosFamilies"
        //            }
        //        );
        //        await this.CreateDatabaseAsync();
        //        await this.CreateContainerAsync();
